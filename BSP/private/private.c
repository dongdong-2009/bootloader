...
const u16 CRC_Table[256]=       //	CCITT	 	X16+X12+X5+1
{
...
u16 crc_table(u8 *ptr,u16 len)
{
    ...
    while(len!=0)
        {
            crcdata=(u8) (CRC_data/256);
            CRC_data<<=8;
            CRC_data^=CRC_Table[crcdata^*ptr];
            ptr++;
            len--;
        }
    return(CRC_data);
    ...
    for(i=1; i<q; i++)
        {
            ...
            else if((*(p_ad+i)==0x5c)&&(*(p_ad+i+1)==0x7b))
                {
                    *(p_ad+i)=0x7c;
                    for(j=i+1; j<q-1; j++)
                        {
